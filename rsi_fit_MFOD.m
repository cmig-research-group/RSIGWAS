...
% Prev Mod: 11/03/17 by Don Hagler
% Last Mod: 10/21/20 by Don Hagler
%
...
function [MFfit,parms] = init_MFfit(vol,qmat,parms)
  ...
  if length(unique(MFfit.bvals))==1 &&...
     length(unique(qlength(qlength>parms.smf)))>1
    MFfit.bvals = MFfit.bvals.*(qlength.^2);
  end;
  MFfit.qmat_orig = qmat;
  MFfit.qmat = bsxfun(@rdivide,qmat,qlength);
